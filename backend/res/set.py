...
def set_background(monitor="both",path="",audio_level="",speed="",scale=""):
    ...
    if monitor !="both":
        ...
        if(indexOfMonitor == 1):
            ...
            command = f"xwinwrap -g {sizes['width']}x{sizes['height']}+0+0 -ni -s -nf -ov  -argb -- mpv --mute=yes --no-audio --no-osc --no-osd-bar --quiet --screen=1 --geometry={sizes['width']}x{sizes['height']}+{sizes['height']}+0 --loop --no-border --video-align-x=1 --video-zoom=1 {path} -wid WID"
    else:
